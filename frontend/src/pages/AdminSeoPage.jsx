...
export default function AdminSeoPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = { title, description, pageType, targetSlug: pageType === 'home' ? '' : targetSlug, isActive, keywords, metaTitle, metaDescription };
        ...
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm animate-fade-in">
          <div className="w-full">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-black text-gray-700 uppercase mb-2">Meta Title</label>
                  <input type="text" className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-orange-500" value={metaTitle} onChange={e => setMetaTitle(e.target.value)} placeholder="SEO Title" />
                </div>
                <div>
                  <label className="block text-xs font-black text-gray-700 uppercase mb-2">Meta Description</label>
                  <input type="text" className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-orange-500" value={metaDescription} onChange={e => setMetaDescription(e.target.value)} placeholder="SEO Description" />
                </div>
              </div>

              <div>
                <label className="block text-xs font-black text-gray-700 uppercase mb-2">Keywords (Press Enter to add)</label>
                <div className="flex flex-wrap gap-2 p-3 border border-gray-200 rounded-xl bg-gray-50 mb-2">
                  {keywords.map((kw, index) => (
                    <span key={index} className="flex items-center gap-1 bg-orange-100 text-orange-800 px-3 py-1 rounded-full text-xs font-bold">
                      {kw}
                      <button type="button" onClick={() => setKeywords(keywords.filter((_, i) => i !== index))} className="text-orange-600 hover:text-orange-800 font-black ml-1">×</button>
                    </span>
                  ))}
                  {keywords.length === 0 && <span className="text-gray-400 text-xs">No keywords added</span>}
                </div>
                <input 
                  type="text" 
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:outline-orange-500" 
                  value={keywordInput} 
                  onChange={e => setKeywordInput(e.target.value)} 
                  onKeyDown={handleAddKeyword}
                  placeholder="Type a keyword and press Enter" 
                />
                <p className="text-[10px] text-gray-400 mt-1">Recommended: 5-15 relevant keywords.</p>
              </div>
