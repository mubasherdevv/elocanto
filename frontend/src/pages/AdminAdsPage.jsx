                    ...
                    <tr key={ad._id} className="hover:bg-gray-50/40 transition-all group">
                        ...
                        <div className="flex items-center gap-5">
                          <div className="relative w-20 h-20 rounded-3xl overflow-hidden bg-gray-100 flex-shrink-0 border-2 border-white shadow-sm transition-transform group-hover:scale-105">
                            {ad.images && ad.images[0] ? (
                              <img
                                src={ad.images[0].startsWith('http') ? ad.images[0] : `http://localhost:5000${ad.images[0]}`}
                                alt={ad.title}
                        ...
                        <div className="flex items-center gap-3">
                          <div className="w-12 h-12 rounded-2xl bg-orange-50 overflow-hidden border border-orange-100 flex-shrink-0">
                            {ad.seller?.profilePhoto ? (
                              <img src={ad.seller.profilePhoto.startsWith('http') ? ad.seller.profilePhoto : `http://localhost:5000${ad.seller.profilePhoto}`} className="w-full h-full object-cover" />
                            ) : (
                      ...
                      <td className="px-8 py-6 text-right">
                        <div className="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0">
                          ...
                          <button
                            onClick={() => renewAd(ad)}
                            className="p-3 bg-yellow-50 border border-yellow-100 rounded-2xl text-yellow-600 hover:text-yellow-700 hover:border-yellow-200 shadow-sm transition-all"
                            title={`Renew (${ad.isFeatured ? (settings?.featuredAdsDuration || 7) : (settings?.simpleAdsDuration || 30)} days)`}
                          >
                  ...
                  <div className="flex flex-col items-center text-center p-6 bg-white rounded-[32px] border border-gray-100 shadow-sm">
                    <div className="w-32 h-32 rounded-[40px] bg-orange-50 overflow-hidden border-4 border-white shadow-xl mb-6">
                      {editingAd.seller?.profilePhoto ? (
                        <img src={editingAd.seller.profilePhoto.startsWith('http') ? editingAd.seller.profilePhoto : `http://localhost:5000${editingAd.seller.profilePhoto}`} className="w-full h-full object-cover" />
                      ) : (
