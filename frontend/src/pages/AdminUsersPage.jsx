...
export default function AdminUsersPage() {
  ...
  const handleCreateOrUpdate = async (e) => {
    ...
    try {
      ...
      setView('list');
      setEditingUser(null);
      setFormData({ name: '', email: '', password: '', phone: '', city: '', isAdmin: false });
      fetchUsers();
    } catch (err) {
                      ...
                      <td className="px-6 py-4 text-center">
                        <button 
                          onClick={() => { 
                            ...
                            setEditingUser(user);
                            setFormData({ name: user.name, email: user.email, phone: user.phone || '', city: user.city || '', isAdmin: user.isAdmin, password: '' });
                      ...
                      {userAds.map(ad => (
                        <div key={ad._id} className="bg-white p-4 rounded-3xl border border-gray-100 shadow-sm flex items-center gap-5 hover:border-[#f95e26]/30 transition-all group">
                          <div className="w-20 h-20 bg-gray-50 rounded-[20px] overflow-hidden border border-gray-100 flex-shrink-0 shadow-inner">
                            {ad.images?.[0] ? (
                              <img src={ad.images[0].startsWith('http') ? ad.images[0] : `http://localhost:5000${ad.images[0]}`} alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                            ) : (
