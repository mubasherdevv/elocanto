...
export default function PublicLayout() {
  ...
  return (
    <div className="flex flex-col min-h-screen pb-[70px] sm:pb-0 relative">
      ...
      {isSidebarOpen && (
        <div 
          className="fixed inset-0 bg-black/50 z-[1000] sm:hidden backdrop-blur-sm transition-opacity"
          onClick={() => setIsSidebarOpen(false)}
        />
      )}

      {/* Mobile Sidebar */}
      <aside className={`fixed top-0 right-0 bottom-0 w-[280px] bg-white z-[1001] sm:hidden transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : 'translate-x-full'} shadow-2xl flex flex-col`}>
